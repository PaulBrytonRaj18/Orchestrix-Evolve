...
function AnalysisCharts({ analysis }: AnalysisChartsProps) {
  if (!analysis) {
    return (
      <div className="empty-state">
        ...
        <p className="empty-description">Run orchestration to generate analysis data</p>
      </div>
    );
  }
  ...
  const maxAuthorCount =
    topAuthors.length > 0 ? Math.max(...topAuthors.map((a) => a.count)) : 1;
      ...
      {analysis.keyword_frequency && analysis.keyword_frequency.length > 0 && (
        <div className="card" style={{ padding: 'var(--space-6)' }}>
          ...
          <div style={{ height: '300px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={analysis.keyword_frequency.slice(0, 15)}
                layout="vertical"
              >
                <CartesianGrid
                  strokeDasharray="3 3"
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  nameKey="bucket"
                >
                  {analysis.citation_distribution.map((_entry, index) => (
                    <Cell
                      key={index}
                      fill={
                        [
