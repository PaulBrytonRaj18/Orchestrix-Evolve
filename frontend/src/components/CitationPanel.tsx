...
function CitationPanel({ papers }: CitationPanelProps) {
  const [selectedStyle, setSelectedStyle] = useState<CitationStyle>('apa');
  const [copiedId, setCopiedId] = useState<string | null>(null);

  const citationStyles: CitationStyle[] = ['apa', 'mla', 'ieee', 'chicago'];

  const copyToClipboard = (text: string, paperId: string): void => {
  ...
  if (!papers || papers.length === 0) {
    return (
      <div className="empty-state">
        ...
        <p className="empty-description">Add papers to your session to generate citations</p>
      </div>
    );
  }
