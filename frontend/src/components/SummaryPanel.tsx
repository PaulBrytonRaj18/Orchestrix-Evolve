...
interface SummaryPanelProps {
  papers: Paper[];
  onSynthesize?: (paperIds: string[]) => void;
}

function SummaryPanel({ papers, onSynthesize }: SummaryPanelProps) {
  const [expandedPapers, setExpandedPapers] = useState<Record<string, boolean>>({});
  const [selectedForSynthesis, setSelectedForSynthesis] = useState<string[]>([]);

  const toggleExpand = (paperId: string): void => {
    setExpandedPapers((prev) => ({ ...prev, [paperId]: !prev[paperId] }));
  };

  const toggleForSynthesis = (paperId: string): void => {
