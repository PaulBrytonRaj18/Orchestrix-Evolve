...
import type { Paper, CitationStyle } from '../types/api';

interface PaperCardProps {
  paper: Paper;
  showNotes?: boolean;
  onNoteChange?: (paperId: string, content: string) => void;
  onCopyCitation?: (text: string) => void;
}

function PaperCard({ paper, showNotes, onNoteChange, onCopyCitation }: PaperCardProps) {
  const [expanded, setExpanded] = useState(false);
  const [showCitationModal, setShowCitationModal] = useState(false);
  const [showSummary, setShowSummary] = useState(false);
  const [copiedFormat, setCopiedFormat] = useState<CitationStyle | null>(null);
  ...
  const truncateAbstract = (text: string | null | undefined, maxLength = 180): string | null => {
    if (!text) return null;
    if (text.length <= maxLength) return text;
    return text.slice(0, maxLength) + '...';
  };

  const handleCopyCitation = (format: CitationStyle, text: string): void => {
  ...
  const citationStyles: CitationStyle[] = ['apa', 'mla', 'ieee', 'chicago'];
    ...
    <>
      <div className="card card-hover" style={{ padding: 'var(--space-5)' }}>
        <div style={{ marginBottom: 'var(--space-4)' }}>
          ...
          >
            <h3 className="paper-title" style={{ flex: 1 }}>
              {paper.title}
            </h3>
          ...
          >
            {formatAuthors(paper.authors)} &bull; {paper.year || 'N/A'} &bull; {paper.source}
          </div>
        </div>
          ...
          >
            ...
            <textarea
              ...
              onBlur={(e: React.FocusEvent<HTMLTextAreaElement>) =>
                onNoteChange && onNoteChange(paper.id, e.target.value)
              }
              style={{
                minHeight: '80px',
                resize: 'vertical',
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() =>
                          paper.citation &&
                          handleCopyCitation(style, paper.citation[style] ?? '')
                        }
                      >
                        {copiedFormat === style ? (
                          <>
